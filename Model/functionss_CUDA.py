...
def partitioned_nearest_neighbors(X, Y, k, metric):#
...
def component_layout(
        data, n_components, component_labels, dim, metric, metric_kwds={}
):
...
def multi_component_layout(
    ...
    if n_components > 2 * dim:
        meta_embedding = component_layout(data, n_components, component_labels, dim, metric=metric,
...
def spectral_layout(data, graph, dim, random_state, metric, metric_kwds={}):
    ...
    if n_components > 1:
        ...
        return multi_component_layout(data, graph, n_components, labels, dim, random_state, metric=metric,
                                      metric_kwds=metric_kwds)
...
def _optimize_layout_euclidean_single_epoch_with_mmd(
        head_embedding,
        head,
        tail,
        n_vertices,
    ...
    for i in range(epochs_per_sample.shape[0]):  # numba.prange
        if epoch_of_next_sample[i] <= n:
            ...
            for d in range(dim):
                grad_d = grad_coeff * (current[d] - other[d]) +  mmd_grad_pos[d]
                grad_d = clip(grad_d)
            ...
            for p in range(n_neg_samples):
                ...
                for d in range(dim):
                    if grad_coeff > 0.0:
                        grad_d = grad_coeff * (current[d] - other[d]) +  mmd_grad_neg[d]
                    else:
                        grad_d = 4.0
                    current[d] += grad_d * alpha
...
def fuzzy_simplicial_set(
    ...
    for i in range(len(Xs)):
        ...
        if Xs[i].shape[0] < 4096:
            ...
            knn_indices, knn_dists, _ = nearest_neighbors(dmat, X_n_neighbors, {}, angular,
                                                          np.random.RandomState(random_state), 'precomputed',
                                                          verbose=verbose)
        else:
            knn_indices, knn_dists, _ = nearest_neighbors(Xs[i], X_n_neighbors, metric_kwds[i], angular,
                                                          np.random.RandomState(random_state), metrics[i],
                                                          verbose=verbose)
...
def init_layout(init,
    ...
    if init == 'random':
        ...
        for i in range(len(Xs)):
            try:
                X_embedding = spectral_layout(Xs[i], graphs[i], n_components, np.random.RandomState(random_state),
                                              metric=metrics[i], metric_kwds=metric_kwds[i])
...
def optimize_layout(
    ...
    joint_epochs_per_negative_sample = {k: joint_epochs_per_sample[k] / negative_sample_rate for k in
                                        joint_graphs.keys()}
    ...
    optimize_fn =numba.njit(_optimize_layout_euclidean_single_epoch_with_mmd
                    , fastmath=True, parallel=parallel
        ...
        for i in range(len(embeddings)):
            ...
            if weights[(i, i)] != 0:
                new_embedding = optimize_fn(
                    ...
                    a,
                    b,
                    np.random.RandomState(random_state).randint(np.iinfo(np.int32).min + 1, np.iinfo(np.int32).max - 1,
                                                                3).astype(np.int64),
                    gamma,
                    dim,
                    move_other,
...
class DNN1(nn.Module):
    ...
    def __init__(self,
        ...
        self.fclayer6 = nn.Sequential(
                nn.Linear(hidden_dims[4], out_features),
                nn.BatchNorm1d(2),
            nn.ReLU()
               )
...
class wrap_gene_location(TensorDataset):
    ...
    def __getitem__(self, index):
        ...
        cordinates = torch.tensor([self.cordx[indexspot],self.cordy[indexspot]])#_norm_norm
    ...
    def train(self, model, train_loader,
        ...
        for epoch in range(num_epochs):
            ...
            for i, img in enumerate(tqdm.tqdm(train_loader)):
                inputs = torch.tensor(img[0]).to(device)
                labels = torch.tensor(img[1]).to(device)
                img = (inputs,labels)
            ...
            print(f'Epoch {epoch + 1}, Learning Rate: {current_lr}, Loss: {total_loss}')
...
def Fit_cord (data_train, out_features,location_data, hidden_dims, num_epochs, batch_size,
    ...
    t_loader= torch.utils.data.DataLoader(DataTra, batch_size=batch_size,
...
def report_prop_method_sc(model, data_test, Val_loader):
    """
    ...
    coords_predict = np.zeros((data_test.shape[0],2))
...
def cal_dist(coord, normalize: bool = True):
    ...
    dist = []
    for i in tqdm.tqdm(range(len(coord))):
        xi, yi = coord[i, :]
